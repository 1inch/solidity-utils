...
Implements efficient safe methods for ERC20 interface.
...
Note that the implementation does not perform dirty bits cleaning, so it is the
...
If permit2 is true, uses the Permit2 standard; otherwise uses the standard ERC20 transferFrom.
...
If `approve(from, to, amount)` fails, it tries to set the allowance to zero, and retries the `approve` call.
Note that the implementation does not perform dirty bits cleaning, so it is the responsibility of
...
Increases with safe math check. Checks if the increased allowance will overflow, if yes, then it reverts the transaction.
...
Decreases with safe math check. Checks if the decreased allowance will underflow, if yes, then it reverts the transaction.
Then uses `forceApprove` to increase the allowance.
Note that the implementation does not perform dirty bits cleaning, so it is the responsibility of
the caller to make sure that the higher 96 bits of the `spender` parameter are clean.
...
Wraps `tryPermit` function and forwards revert reason if permit fails.
...
Invokes `tryPermit` with sender as owner and contract as spender.
...
The function is designed to support a variety of permit functions, namely: IERC20Permit, IDaiLikePermit, IERC7597Permit and IPermit2.
...
Uses inline assembly to interact with the IWETH contract.
