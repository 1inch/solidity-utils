...
describe('SafeERC20', async () => {
    ...
    describe('with address that has no contract code', async () => {
        shouldRevertOnAllCalls(['SafeTransferFailed', 'SafeTransferFromFailed', 'ForceApproveFailed', ''], false);
    });

    describe('with token that returns false on all calls', async () => {
        shouldRevertOnAllCalls(['SafeTransferFailed', 'SafeTransferFromFailed', 'ForceApproveFailed'], true);
    });

    describe('with token that returns true on all calls', async () => {
        shouldOnlyRevertOnErrors(true);
    });

    describe('with token that returns no boolean values', async () => {
        shouldOnlyRevertOnErrors(false);
    });
    ...
    function shouldRevertOnAllCalls(reasons: string[], isFalseMock: boolean) {
        it('reverts on transfer', async () => {
            const wrapper = await loadFixture(isFalseMock ? deployWrapperFalseMock : deployWrapperSimple);
    ...
    function shouldOnlyRevertOnErrors(isTrueMock: boolean) {
        it("doesn't revert on transfer", async () => {
            const wrapper = await loadFixture(isTrueMock ? deployWrapperTrueMock : deployWrapperNoReturnMock);
