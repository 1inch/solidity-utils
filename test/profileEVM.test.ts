...
describe('trace inspection', function () {
    ...
    describe('profileEVM', function () {
        it('should be counted ERC20 Transfer', async function () {
            ...
            expect(await profileEVM(txn.hash, ['STATICCALL', 'CALL', 'SSTORE', 'SLOAD'], ethers.provider)).to.be.deep.equal([
                0, 0, 2, 2,
            ]);
        });
    ...
    describe('gasspectEVM', function () {
        ...
        it('should be counted ERC20 Transfer with minOpGasCost = 2000', async function () {
            ...
            const txn = await usdt.transfer(signer2.address, ether('1'));
            expect(await gasspectEVM(txn.hash, { minOpGasCost: 2000 }, ethers.provider)).to.be.deep.equal([
                '0-0-SLOAD = 2100',
                '0-0-SSTORE = 2900',
                '0-0-SLOAD = 2100',
                '0-0-SSTORE = 2900',
        ...
        it('should be counted ERC20 Transfer with args', async function () {
            ...
            const txn = await usdt.transfer(signer2.address, ether('1'));
            expect(await gasspectEVM(txn.hash, { args: true }, ethers.provider)).to.be.deep.equal([
        ...
        it('should be counted ERC20 Transfer with res', async function () {
            ...
            const txn = await usdt.transfer(signer2.address, ether('1'));
            expect(await gasspectEVM(txn.hash, { res: true }, ethers.provider)).to.be.deep.equal([
